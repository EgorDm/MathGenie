...
namespace Hero
{
	/// Hero::eval() throws a Hero::error if it fails
			...
			if (value == 0)
			{
				...
				if (division != std::string::npos)
					msg << " (error token is \""
						...
						<< "\")";
				throw Hero::error(expr_, msg.str());
			}
			return value;
		}
		...
		T parseValue()
		{
			...
			case '(': index_++;
				...
				if (getCharacter() != ')')
				{
					if (!isEnd())
						unexpected();
					throw Hero::error(expr_, "Syntax error: `)' expected at end of expression");
				}
				index_++;
				break;
			case '~': index_++;
			...
			default: if (!isEnd())
					unexpected();
				throw Hero::error(expr_, "Syntax error: value expected at end of expression");
			}
			return val;
		}
